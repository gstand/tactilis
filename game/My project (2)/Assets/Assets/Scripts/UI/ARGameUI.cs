...
public class ARGameUI : MonoBehaviour {
    ...
    public Image countdownPanelBackground;
    ...
    void Start() {
        ...
        if (countdownPanelBackground != null) {
            countdownPanelBackground.color = backgroundColor;
        }
    ...
    public void ShowTimer(bool show) {
        if (timerPanel != null) {
            timerPanel.gameObject.SetActive(show);
        }
    }
    ...
    public void ShowCountdown(bool show) {
        if (countdownPanel != null) {
            countdownPanel.gameObject.SetActive(show);
        }
    }
    
    void UpdateCountdownPanelSize() {
        if (countdownPanel == null || countdownText == null) return;
        
        countdownText.ForceMeshUpdate();
        
        Vector2 textSize = new Vector2(
            countdownText.preferredWidth + paddingHorizontal * 2,
            countdownText.preferredHeight + paddingVertical * 2
        );
        
        countdownPanel.sizeDelta = textSize;
    }
}
